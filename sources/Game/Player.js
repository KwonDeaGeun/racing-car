...
export class Player {
	...
	constructor() {
		...
		this.game.physicalVehicle.chassis.physical.initialState.position.x =
			respawn.position.x;
		this.game.physicalVehicle.chassis.physical.initialState.position.y =
			respawn.position.y;
		this.game.physicalVehicle.chassis.physical.initialState.position.z =
			respawn.position.z;
	...
	setSounds() {
		...
		// Wheels on floor
		{
			...
			this.game.audio.register({
				...
				onPlaying: (item) => {
					...
					const speedEffect = Math.min(
						1,
						this.game.physicalVehicle.xzSpeed * 0.1,
					);
			...
			this.game.audio.register({
				...
				onPlaying: (item) => {
					const directionRatio =
						(1 - Math.abs(this.game.physicalVehicle.forwardRatio)) * 0.6;
		...
		this.game.audio.register({
			...
			onPlaying: (item) => {
				const accelerating =
					0.5 + Math.abs(this.game.player.accelerating) * 0.5;
	...
	setInputs() {
		this.game.inputs.addActions([
			...
			},
			{
				...
				keys: ["Keyboard.KeyR", "Gamepad.select"],
			},
			{
				name: "suspensions",
				categories: ["wandering", "racing"],
			...
			},
			{
				...
				categories: ["wandering", "racing"],
				keys: ["Keyboard.Numpad8"],
			},
			{
				name: "suspensionsBack",
				categories: ["wandering", "racing"],
				keys: ["Keyboard.Numpad2"],
			},
			{
				name: "suspensionsRight",
				categories: ["wandering", "racing"],
				keys: ["Keyboard.Numpad6", "Gamepad.r1"],
			},
			{
				name: "suspensionsLeft",
				categories: ["wandering", "racing"],
				keys: ["Keyboard.Numpad4", "Gamepad.l1"],
			},
			{
	...
	setDistanceDriven() {
		...
		this.distanceDriven.value = localDistanceDriven
			? parseInt(localDistanceDriven)
			: 0;
	...
	updatePrePhysics() {
		 ...
		 */
		if (
			this.game.inputs.nipple.active &&
			this.game.inputs.nipple.progress > 0
		) {
			...
			const angleDeltaAbsNormalized =
				angleDeltaAbs /
				((Math.PI * 2 - this.game.inputs.nipple.forwardAmplitude) / 2);
	...
	updatePostPhysics() {
		...
		if (
			this.boosting &&
			this.accelerating &&
			this.game.physicalVehicle.speed > 15
		)
		...
		this.rotationY = Math.atan2(
			this.game.physicalVehicle.forward.z,
			this.game.physicalVehicle.forward.x,
		);
		...
		if (this.game.physicalVehicle.wheels.justTouchedCount > 1) {
			this.sounds.spring1.play(
				this.game.physicalVehicle.wheels.justTouchedCount,
			);
			this.sounds.spring2.play(
				this.game.physicalVehicle.wheels.justTouchedCount,
			);
		}
		...
		if (
			!this.timePlayed.achieved &&
			this.timePlayed.session > this.game.dayCycles.duration
		) {
		...
		this.distanceDriven.value +=
			this.game.physicalVehicle.xzSpeed * this.game.ticker.deltaScaled;
		...
		if (
			...
			distanceDrivenKm >
				this.game.achievements.groups.get("distanceDriven").progress
		) {
