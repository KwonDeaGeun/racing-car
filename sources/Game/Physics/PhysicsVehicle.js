...
export class PhysicsVehicle {
	constructor() {
		...
		if (this.game.debug.active) {
			...
			this.debugPanel.addBinding(this, "steeringAmplitude", {
				min: 0,
				max: Math.PI * 0.5,
				step: 0.01,
			});
			this.debugPanel.addBinding(this, "engineForceAmplitude", {
				min: 1,
				max: 20,
				step: 1,
			});
			this.debugPanel.addBinding(this, "boostMultiplier", {
				min: 1,
				max: 5,
				step: 0.01,
			});
			this.debugPanel.addBinding(this, "topSpeed", {
				min: 0,
				max: 20,
				step: 0.1,
			});
			this.debugPanel.addBinding(this, "topSpeedBoost", {
				min: 0,
				max: 20,
				step: 0.1,
			});
			this.debugPanel.addBinding(this, "brakeAmplitude", {
				min: 0,
				max: 200,
				step: 0.01,
			});
			this.debugPanel.addBinding(this, "idleBrake", {
				min: 0,
				max: 1,
				step: 0.001,
			});
			this.debugPanel.addBinding(this, "reverseBrake", {
				min: 0,
				max: 1,
				step: 0.001,
			});

			this.debugPanel.addBinding(this.suspensionsHeights, "low", {
				min: 0,
				max: 2,
				step: 0.01,
			});
			this.debugPanel.addBinding(this.suspensionsHeights, "mid", {
				min: 0,
				max: 2,
				step: 0.01,
			});
			this.debugPanel.addBinding(this.suspensionsHeights, "high", {
				min: 0,
				max: 2,
				step: 0.01,
			});

			this.debugPanel.addBinding(this.suspensionsStiffness, "low", {
				min: 0,
				max: 100,
				step: 0.1,
			});
			this.debugPanel.addBinding(this.suspensionsStiffness, "mid", {
				min: 0,
				max: 100,
				step: 0.1,
			});
			this.debugPanel.addBinding(this.suspensionsStiffness, "high", {
				min: 0,
				max: 100,
				step: 0.1,
			});
		...
		this.setChassis();
		this.controller = this.game.physics.world.createVehicleController(
	...
	setChassis() {
		...
		const object = this.game.objects.add(null, {
			...
			onCollision: (force, position) => {
				this.game.audio.groups
					.get("hitDefault")
					.playRandomNext(force, position);
			},
		});
	...
	setWheels() {
		...
		this.wheels.updateSettings = () => {
			...
			for (const wheel of this.wheels.items) {
				...
				this.controller.setWheelDirectionCs(
					i,
					this.wheels.settings.directionCs,
				);
				...
				this.controller.setWheelMaxSuspensionForce(
					i,
					this.wheels.settings.maxSuspensionForce,
				);
				this.controller.setWheelMaxSuspensionTravel(
					i,
					this.wheels.settings.maxSuspensionTravel,
				);
				this.controller.setWheelSideFrictionStiffness(
					i,
					this.wheels.settings.sideFrictionStiffness,
				);
				this.controller.setWheelSuspensionCompression(
					i,
					this.wheels.settings.suspensionCompression,
				);
				this.controller.setWheelSuspensionRelaxation(
					i,
					this.wheels.settings.suspensionRelaxation,
				);
		...
		if (this.game.debug.active) {
			...
			this.debugPanel
				.addBinding(this.wheels.settings, "offset", {
					min: -1,
					max: 2,
					step: 0.01,
				})
			...
			this.debugPanel
				.addBinding(this.wheels.settings, "radius", {
					min: 0,
					max: 1,
					step: 0.01,
				})
				.on("change", this.wheels.updateSettings);
			this.debugPanel
				.addBinding(this.wheels.settings, "frictionSlip", {
					min: 0,
					max: 1,
					step: 0.01,
				})
				.on("change", this.wheels.updateSettings);
			this.debugPanel
				.addBinding(this.wheels.settings, "maxSuspensionForce", {
					min: 0,
					max: 1000,
					step: 1,
				})
				.on("change", this.wheels.updateSettings);
			this.debugPanel
				.addBinding(this.wheels.settings, "maxSuspensionTravel", {
					min: 0,
					max: 2,
					step: 0.01,
				})
				.on("change", this.wheels.updateSettings);
			this.debugPanel
				.addBinding(this.wheels.settings, "sideFrictionStiffness", {
					min: 0,
					max: 10,
					step: 0.01,
				})
				.on("change", this.wheels.updateSettings);
			this.debugPanel
				.addBinding(this.wheels.settings, "suspensionCompression", {
					min: 0,
					max: 30,
					step: 0.01,
				})
				.on("change", this.wheels.updateSettings);
			this.debugPanel
				.addBinding(this.wheels.settings, "suspensionRelaxation", {
					min: 0,
					max: 10,
					step: 0.01,
				})
				.on("change", this.wheels.updateSettings);
			this.debugPanel
				.addBinding(this.wheels.settings, "suspensionStiffness", {
					min: 0,
					max: 100,
					step: 0.1,
				})
	...
	setUpsideDown() {
		...
		this.upsideDown.test = () => {
			this.upsideDown.ratio =
				this.upward.dot(new THREE.Vector3(0, -1, 0)) * 0.5 + 0.5;
	...
	setFlip() {
		...
		this.flip.jump = () => {
			...
			const impulse = new THREE.Vector3(0, 1, 0).multiplyScalar(
				this.flip.force * this.chassis.mass,
			);
			this.chassis.physical.body.applyImpulse(impulse);

			// Upside down
			if (
				upwarddAbsolute > sidewardAbsolute &&
				upwarddAbsolute > forwardAbsolute
			) {
		...
		if (this.game.debug.active) {
			this.debugPanel.addBinding(this.flip, "force", {
				label: "flipForce",
				min: 0,
				max: 10,
				step: 0.01,
	...
	updatePrePhysics() {
		...
		if (
			!this.game.player.braking &&
			Math.abs(this.game.player.accelerating) < 0.1
		)
		...
		for (let i = 0; i < 4; i++) {
			...
			this.controller.setWheelSuspensionRestLength(
				i,
				this.suspensionsHeights[this.game.player.suspensions[i]],
			);
			this.controller.setWheelSuspensionStiffness(
				i,
				this.suspensionsStiffness[this.game.player.suspensions[i]],
			);
			...
			if (groundObject && this.game.world.waterSurface) {
				const onIce =
					groundObject.parent() ===
					this.game.world.waterSurface.ice.physical.body;
				...
				this.controller.setWheelFrictionSlip(
					i,
					onIce ? iceFriction : this.wheels.settings.frictionSlip,
				);
			}
		}
		...
		// Update controller
		const delta =
			this.game.quality.level === 1
				? 1 / 60
				: Math.min(1 / 60, this.game.ticker.deltaAverage);
	...
	updatePostPhysics() {
		...
		const newPosition = new THREE.Vector3().copy(
			this.chassis.physical.body.translation(),
		);
		...
		this.xRotation = new THREE.Euler().setFromQuaternion(
			this.quaternion,
			"XYZ",
		).x;
		this.yRotation = new THREE.Euler().setFromQuaternion(
			this.quaternion,
			"YXZ",
		).y;
		this.zRotation = new THREE.Euler().setFromQuaternion(
			this.quaternion,
			"ZYX",
		).z;

		if (Math.abs(this.game.player.accelerating) > 0.5)
			this.stuck.accumulate(
